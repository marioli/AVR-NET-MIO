  ...
  do
  {
    ...
    switch (dhcp_state)
    {
      ...
      case DHCP_STATE_ACK_RCVD:
        ...
        (*((unsigned long*)&ntp_server_ip[0])) = (*((unsigned long*)&cache.ntp_ip[0]));
  ...
  do
  {
    switch (msg[ix])
    {
      ...
      case 6: //dns len = n * 4
        ...
        if ( len >= 8 )
        {
          /* two or more dns servers in response */
          ix++;
          get4bytes(&msg[ix], &c->dns1_ip[0]);
          get4bytes(&msg[ix + 4], &c->dns2_ip[0]);
          ix += len;
        }
        else
        {
      ...
      case 42: //ntp server len = n * 4
        ix++;
        len = msg[ix];
        if(len >= 4)
        {
          ix++;
          get4bytes(&msg[ix], &c->ntp_ip[0]);
          ix += len;
        }
        else
        {
      ...
      default: 
        DHCP_DEBUG("Unknown Option: %2x len: %d\r\n", msg[ix], msg[ix+1]);
        ix++;
        ix += (msg[ix]+1);
      break;
    }
